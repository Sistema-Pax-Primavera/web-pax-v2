  ...
  const filtro = () => {
    ...
    if (isClienteLigar) {
      ...
      clientesFiltrados = clientesFiltrados.filter(cliente => {
        ...
        for (const agendamento of agendamentos) {
          const agendamentoTel = agendamento.ult_agendamento_tel ? converterDataParaFormatoBackend(agendamento.ult_agendamento_tel) : null;
          const mesAgendamento = agendamentoTel ? new Date(agendamentoTel).getMonth() + 1 : null;
          if (
            (!agendamentoTel || (mesAgendamento && mesAgendamento < mesAtual))
          ) {
            return true;
          }
        }
        ...
        clientesFiltrados = clientesFiltrados.filter(cliente => {
          if (cliente.dados_cliente.observacao) {
            return cliente.dados_cliente.observacao.some(obs => {
              const dataUltJustificativa = converterDataParaFormatoBackend(obs.data_criacao);
    ...
    if (isAgendamentoDia) {
      clientesFiltrados = clientesFiltrados.filter(cliente => {
        const agendamentos = cliente.dados_cliente.agendamentos || [];
        for (const agendamento of agendamentos) {
          const agendamentoCob = agendamento.ult_agendamento_cob ? converterDataParaFormatoBackend(agendamento.ult_agendamento_cob) : null;
          const agendamentoTel = agendamento.ult_agendamento_tel ? converterDataParaFormatoBackend(agendamento.ult_agendamento_tel) : null;
          if (
            agendamentoCob && agendamentoCob >= dataInicio && agendamentoCob <= dataFinal ||
            agendamentoTel && agendamentoTel >= dataInicio && agendamentoTel <= dataFinal
          ) {
            return true;
          }
        }
